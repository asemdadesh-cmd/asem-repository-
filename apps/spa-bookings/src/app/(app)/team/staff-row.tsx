...
import { setLockboxAccess, setStaffActive, setStaffRole } from "@/app/actions/admin";
    ...
    <div className={cx("px-4 py-3", !person.is_active && "opacity-60")}>
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {person.role !== "admin" && person.can_edit_lockbox && (
            <Badge tone="ready">Lockbox</Badge>
          )}
      ...
      {isAdmin && !isSelf && (
        <div className="mt-2.5 flex flex-wrap gap-2">
          ...
          {person.role !== "admin" && (
            <ActionForm
              action={setLockboxAccess}
              fields={{
                user_id: person.id,
                can_edit_lockbox: String(person.can_edit_lockbox),
              }}
              label={
                person.can_edit_lockbox
                  ? "Remove lockbox access"
                  : "Let them change the lockbox code"
              }
              pendingLabel="Updating…"
              size="sm"
              variant="secondary"
